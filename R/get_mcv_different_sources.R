...
get_mcv_different_sources <- function(mcv1_annualized_all_time, mcv2_annualized_all_time){
  ...
  mcv_annualized_f <-  bind_rows( mcv1_annualized_all_time, mcv2_annualized_all_time)%>%
    ...
    select(-c( Months, LGA)) %>%
    mutate(Indicator = as.character(case_when(Indicator == "Measles 1 Dose Coverage (Annualized)" ~ "Dhis2 (MCV1)",
                                              Indicator == "Measles 2 Dose Coverage (Annualized)" ~ "Dhis2 (MCV2)"))) %>%
  ...
  mcv_different_sources %>%
    full_join(mcv_annualized_f, by=c("Year" = "Year")) %>%
    arrange(Year) %>%
